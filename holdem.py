...
if __name__ == '__main__':
    ...
    if len(args.Hole_Cards) > 0:
        sim = s.simulation(args.Hole_Cards, args.flop, args.turn, args.river)
        pair_pct = s.percent(sim[1], sim[0])
        pair_ratio = s.ratio(sim[1], sim[0])
        two_pair_pct = s.percent(sim[2], sim[0])
        two_pair_ratio = s.ratio(sim[2], sim[0])
        three_ok_pct = s.percent(sim[3], sim[0])
        three_ok_ratio = s.ratio(sim[3], sim[0])
        straight_pct = s.percent(sim[4], sim[0])
        straght_ratio = s.ratio(sim[4], sim[0])
        flush_pct = s.percent(sim[5], sim[0])
        flush_ratio =  s.ratio(sim[5], sim[0])
        boat_pct = s.percent(sim[6], sim[0])
        boat_ratio = s.ratio(sim[6], sim[0])
        quads_pct = s.percent(sim[7], sim[0])
        quads_ratio = s.ratio(sim[7], sim[0])
        strt_flush_pct = s.percent(sim[8], sim[0])
        strt_flush_ratio = s.ratio(sim[8], sim[0])
